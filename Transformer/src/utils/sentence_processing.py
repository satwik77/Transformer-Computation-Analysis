...
def pad_seq(seq, max_length, voc, pad):
	seq += [voc.get_id(pad) for i in range(max_length - len(seq))]
	return seq
...
def process_batch(sent1s, sent2s, voc1, voc2, device, pad):
	input_len1 = [len(s) for s in sent1s]
	input_len2 = [len(s) for s in sent2s]
	max_length_1 = max(input_len1)
	max_length_2 = max(input_len2)

	sent1s_padded = [pad_seq(s, max_length_1, voc1, pad) for s in sent1s]
	sent2s_padded = [pad_seq(s, max_length_2, voc2, pad) for s in sent2s]
